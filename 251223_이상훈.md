...
**예시 코드 1 (ref 없음) 동작 차이 설명 :**

 `Modify` 함수는 배열의 **참조값(주소)** 을 값으로 복사하여 받는다. `arr[1] = 500;`은 주소를 따라가서 `힙(Heap)` 영역의 실제 데이터를 수정하는 행위이므로 `원본 배열에 영향`이 있다.
...
**예시 코드 2 (ref 있음) 동작 차이 설명 :**

`ModifyRef` 함수는 `ref` 키워드를 사용해 `numArr` 변수 자체의 **참조(Reference)**를 받는다. 즉, 함수 내부의 `arr`는 `numArr`와 **완전히 동일한 변수로 취급**된다(별명처럼 동작). ∴ `arr = new int[] { 7, 7, 7 };`를 실행하면, 원본 변수 `numArr`가 가리키는 대상 자체가 `새로운 배열로 변경`된다. 그래서 출력 결과가 새로운 배열의 값인 `7`이 된다.