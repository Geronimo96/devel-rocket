...
* 카드 덱을 책상 위에 쌓아두었다고 상상해 보세요. 우리는 항상 **맨 위(Top)**에 있는 카드를 집어갑니다.
* `Stack`은 데이터를 쌓아 올리고(Push), 가장 위에 있는 데이터를 꺼내는(Pop) 구조이므로, **"덱의 맨 위에서 카드를 뽑는다"**는 개념과 정확히 일치합니다.


2. **기능 매칭**:
* **Peek (보기만 하기)**: `ShowTopCard` 기능은 카드를 제거하지 않고 확인만 해야 합니다. 스택의 `Peek()` 메서드가 이 역할을 완벽히 수행합니다.
* **Pop (꺼내기)**: `DrawCard` 기능은 카드를 가져오면서 덱에서 제거해야 합니다. 스택의 `Pop()` 메서드가 이 역할을 수행합니다.
* **Push (쌓기)**: 뽑은 카드를 `usedCards` 더미에 버릴 때도, 바닥에 버려진 카드 위에 계속 쌓이게 되므로 스택 구조가 적합합니다.

### 📝 구현 핵심 포인트

1. **초기화 및 셔플 (Constructor)**:
* `Stack` 자체는 인덱스로 접근하여 섞기(Swap)가 어렵습니다.
* 따라서 생성자에서 임시로 **`List<Card>`**를 만들어 52장을 생성한 뒤, 리스트 상태에서 `Shuffle`을 수행하고, 그 후에 `unusedCards` 스택에 `Push`하는 방식을 사용했습니다.


2. **`unusedCards` -> `usedCards` 이동**:
* `DrawCard()` 메서드 내에서 `unusedCards.Pop()`으로 꺼낸 객체를 리턴함과 동시에 `usedCards.Push()`를 수행하여 카드가 사라지지 않고 사용된 덱으로 이동하도록 구현했습니다.
